...
{
        (data())->in_child++;
        if (struc->input >= 0 && dup2(struc->input, STDIN_FILENO) < 0 && exit_exec(1))
                exit(exit_properly((data())->exit_code));
        if (struc->output >= 0 && dup2(struc->output, STDOUT_FILENO) < 0 && exit_exec(1))
                exit(exit_properly((data())->exit_code));
        if ((!fd_update(&struc->to_close[P_RD], -1) || !fd_update(&struc->to_close[P_WR], -1)) && exit_exec(1))
                exit(exit_properly((data())->exit_code));
        if ((!fd_update(&struc->out_pipe[P_RD], -1) || !fd_update(&struc->out_pipe[P_WR], -1)) && exit_exec(1))
                exit(exit_properly((data())->exit_code));
