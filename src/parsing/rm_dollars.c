...
int go_til_end(int from_hrd)
{
        ...
        while ((data())->p_index->str[++(data())->i])
                if ((data())->p_index->str[(data())->i] == '$' && !(data())->p_index->no_extend && set_int(&(data())->j, (data())->i, 1) && (from_hrd || (!from_hrd && was_quoted((data())->p_index->quotes, (data())->i) != S_QUOTE)))
                        if (!dollar_finded(1))
                                return (0);
        (data())->i = 0;
        while ((data())->p_index->str[(data())->i])
...
{
        if (!from_hrd && (data())->p_index && !(data())->p_index->str[0] && !(data())->p_index->was_quoted)
        {
