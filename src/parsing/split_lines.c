...
int redir_finded(void)
{
        ...
        (data())->j++;
        while ((data())->line[(data())->j] && ((data())->line[(data())->j] == '>' || (data())->line[(data())->j] == '<'))
                (data())->j++;
        return (1);
}
...
int next_str(int quote)
{
        while ((data())->line[(data())->j] && (data())->line[(data())->j] != ' ' && (data())->line[(data())->j] != '\t')
        {
                ...
                if ((data())->line[(data())->j] == '>' || (data())->line[(data())->j] == '<')
                        return (redir_finded());
                if ((data())->line[(data())->j] == D_QUOTE || (data())->line[(data())->j] == S_QUOTE)
                {
                        quote = (data())->line[(data())->j];
                        (data())->j++;
                        while ((data())->line[(data())->j] && (data())->line[(data())->j] != quote)
                                (data())->j++;
                }
                if ((data())->line[(data())->j] && (data())->line[(data())->j] != ' ' && (data())->line[(data())->j] != '\t')
                        (data())->j++;
        }
        return (1);
}
...
int go_til_space(void)
{
        ...
        if ((data())->line[(data())->i] && ((data())->line[(data())->i] == ' ' || (data())->line[(data())->i] == '\t'))
                (data())->i++;
        return (1);
}
...
int split_line(void)
{
        ...
        while ((data())->line && (data())->line[(data())->i])
                go_til_space();
        if (!(data())->p_start && (data())->line && ((data())->line[0] == ' ' || (data())->line[0] == '\t'))
        {
                if (!new_p_struct_member())
                        return (0);
