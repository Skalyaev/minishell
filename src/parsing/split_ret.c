...
{
        while ((data())->p_index->str[i] && ((data())->p_index->str[i] == ' ' || (data())->p_index->str[i] == '\t'))
                i++;
...
int split_ret(void)
{
        while (--(data())->i >= 0)
        {
                if (((data())->p_index->str[(data())->i] == ' ' || (data())->p_index->str[(data())->i] == '\t') && !was_quoted((data())->p_index->quotes, (data())->i))
                {
                        if (from_dollar((data())->p_index->dollars, (data())->i))
                                (data())->p_index->no_extend++;
                        while ((data())->i >= 0 && ((data())->p_index->str[(data())->i] == ' ' || (data())->p_index->str[(data())->i] == '\t') && !was_quoted((data())->p_index->quotes, (data())->i))
                                (data())->i--;
                        (data())->i++;
