...
int check_for_insta_return(void)
{
        ...
        if (!(data())->p_index->str[(data())->j + 1] || (!ft_isalnum((data())->p_index->str[(data())->j + 1]) && (data())->p_index->str[(data())->j + 1] != '_' && (data())->p_index->str[(data())->j + 1] != '?'))
                return (1);
...
int dollar_finded(int ret)
{
        ...
        {
                (data())->j++;
                if (!(data())->p_index->str[(data())->j] || (!ft_isalnum((data())->p_index->str[(data())->j]) && (data())->p_index->str[(data())->j] != '_') || was_quoted((data())->p_index->quotes, (data())->j) != (data())->old)
                {
