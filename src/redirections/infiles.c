...
int fill_infile(t_parsing *cursor, t_exec *struc, int redir)
{
        ...
        if (!fd_update(&struc->input, redir) || !fd_update(&struc->to_close[P_WR], -1))
                return (0);
        return (1);
}
...
{
        if (!fd_update(&(data())->he_pipe[P_RD], -1) || !fd_update(&(data())->he_pipe[P_WR], -1))
                return (0);
        if ((data())->he_read)
        {
...
int fill_heredoc(t_exec *struc, t_parsing *htmp, int i)
{
        ...
        if (!fd_update(&(struc->to_close[P_WR]), (data())->he_pipe[P_WR]) || !fd_update(&(struc->input), (data())->he_pipe[P_RD]))
                return (0);
        return (1);
}
