...
#    Updated: 2022/08/13 16:07:21 by anguinau         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
INCLUDESDIR		=	include
OBJSDIR			=	obj
OBJS_SUBDIR		=	$(shell find $(SRCSDIR) -type d | grep '/' | sed 's/src/obj/g')
...
LIB			=	libft/libft.a
L_SRCS			=	$(shell find libft/srcs -type f -name "*.c")
L_INCLUDES		=	$(shell find libft/includes -type f -name "*.h")
L_OBJS			=	$(subst libft/srcs,libft/objs,$(L_SRCS:.c=.o))
...
$(NAME)			:	$(LIB) $(L_SRCS) $(L_OBJS) $(L_INCLUDES) $(OBJSDIR) $(OBJS_SUBDIR) $(OBJS) 
				@${CC} $(CFLAGS) -o ${NAME} ${OBJS} -L. $(LIB) -lreadline 
...
$(OBJS_SUBDIR)		:
				...
				@$(eval BUILD_SIZE=$(shell echo $$(($(BUILD_SIZE)+1))))
				@echo -ne '	                                                \r'
				@echo -e '	obj directory $(_GREEN)created$(_STOP).'
				@echo -ne '	$(E_BAR)\r'
				@$(eval $(call update_bar))
